...
def analyze_sma(symbol: str) -> Optional[str]:
    ...
    fast_w = int(wins["fast"])
    slow_w = int(wins["slow"])
    buy_th = float(th["buy"])
    sell_th = float(th["sell"])
    hold_band = float(th["hold"])

    print(f"[SMA] Analyzing {symbol} ({bucket})")

    df = get_clean_data(symbol, days=120)
